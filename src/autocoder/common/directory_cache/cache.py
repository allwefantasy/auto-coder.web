...
class DirectoryCache:
    ...
    def _build(self) -> None:
        ...
        for r, ds, fs in os.walk(self.root, followlinks=True):
            # 过滤掉需要忽略的目录
            ds[:] = [d for d in ds if not should_ignore(os.path.join(r, d), root_path=self.root)]
            for f in fs:
                fp = os.path.join(r, f)
                abs_fp = os.path.abspath(fp)
                if not should_ignore(abs_fp, root_path=self.root):
    ...
    async def _on_change(self, change: Change, path: str) -> None:
        ...
        if should_ignore(ap, root_path=self.root):
