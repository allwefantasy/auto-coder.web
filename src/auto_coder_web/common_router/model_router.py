...
async def get_models():
    ...
    """
    try:
        models_list = models.load_models()
        return models_list
...
async def add_model(model: Model):
    ...
    try:
        ...
        models.save_models(existing_models)
        return model
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_model(model_name: str):
    ...
    try:
        ...
        models.save_models(models_list)
...
async def update_model_api_key(model_name: str, api_key: str):
    ...
    """
    try:
        result = models.update_model_with_api_key(model_name, api_key)
        if result:
...
async def update_model_input_price(model_name: str, price: float):
    ...
    """
    try:
        result = models.update_model_input_price(model_name, price)
        if result:
...
async def update_model_output_price(model_name: str, price: float):
    ...
    """
    try:
        result = models.update_model_output_price(model_name, price)
        if result:
...
async def update_model_speed(model_name: str, speed: float):
    ...
    """
    try:
        result = models.update_model_speed(model_name, speed)
        if result:
