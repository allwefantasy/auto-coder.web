...
import React, { useState, useEffect } from 'react';
import { Switch, InputNumber, Select, message } from 'antd';
import { getMessage } from '../Sidebar/lang';
import type { AutoCoderArgs } from './types';
import './ModelConfig.css';
...
const BasicSettings: React.FC<BasicSettingsProps> = ({ availableKeys, onSettingChange }) => {
  ...
  useEffect(() => {
    const fetchCurrentConfig = async () => {
      ...
      try {
        ...
        if (currentConfig.index_filter_model_max_input_length !== undefined) {
          updatedSettings.index_filter_model_max_input_length = 
            parseInt(currentConfig.index_filter_model_max_input_length) || 51200;
        }
        if (currentConfig.auto_merge !== undefined) {
          updatedSettings.auto_merge = currentConfig.auto_merge || 'editblock';
        }
  ...
  useEffect(() => {
    ...
    availableKeys.forEach(key => {
      if (key.key === 'index_filter_model_max_input_length' && initialSettings.index_filter_model_max_input_length === undefined) {
        initialSettings.index_filter_model_max_input_length = parseInt(key.default) || 51200;
      }
      if (key.key === 'auto_merge' && initialSettings.auto_merge === undefined) {
        initialSettings.auto_merge = key.default || 'editblock';
      }     
    ...
    <div>
      ...
      <div className="space-y-4">
        <div className="model-config-item">
          ...
          <p className="model-config-description">如果索引大于该值，则会进行拆分处理</p>
        </div>

        <div className="model-config-item">
          <div className="flex justify-between items-center">
            <label className="model-config-label">自动合并方式</label>
            <Select
              value={settings.auto_merge}
              onChange={(value) => handleSettingChange('auto_merge', value)}
              className="w-32"
          ...
          <p className="model-config-description">选择代码自动合并的方式</p>
        </div>
      </div>
    </div>
  );
