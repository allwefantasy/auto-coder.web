...
const EditablePreviewPanel: React.FC<EditablePreviewPanelProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      debouncedPreviewUrl &&
      debouncedPreviewUrl !== "http://127.0.0.1:3000" &&
      !initialUrl
    ) {
  ...
  const sendCommandToIframe = useCallback(
    (command: string, payload?: any) => {
      if (iframeRef.current && iframeRef.current.contentWindow) {
        if (
          !isIframeReady &&
          command !== "getDOM" &&
          command !== "disableEditing"
        ) {
          ...
          console.warn(
            `EditablePreviewPanel: Iframe might not be fully ready for command '${command}'.`
          );
        ...
        try {
          ...
          console.error(
            "Could not determine iframe target origin from src:",
            iframeSrc,
            e
          );
          message.error(
            "Error determining iframe origin. Cannot send command securely."
          );
        ...
        console.log(
          `EditablePreviewPanel: Sent command '${command}' to iframe. Target Origin: ${targetOrigin}`
        );
      } else {
        console.warn(
          `EditablePreviewPanel: Cannot send command '${command}'. Iframe ref or contentWindow missing.`
        );
        message.error(
          `Cannot send command '${command}'. Preview window not available.`
        );
      }
    },
    [iframeSrc, isIframeReady]
  ); // Depend on iframeSrc to get the correct targetOrigin and readiness
  ...
  const saveDOM = async (htmlContent: string) => {
    ...
    console.log(
      "EditablePreviewPanel: Received DOM content, attempting to save..."
    );
    ...
    try {
      ...
      message.success(
        `Preview saved successfully! (ID: ${response.data.save_id})`
      );
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        console.error(
          "Could not determine expected origin from iframe src:",
          iframeSrc,
          e
        );
      ...
      if (event.origin !== expectedOrigin) {
        ...
        if (
          ...
          console.warn(
            `EditablePreviewPanel: Ignored message from unexpected origin: ${event.origin}. Expected: ${expectedOrigin}`
          );
          return;
        }
      }
        ...
        switch (data.type) {
          case "status":
            if (data.status === "ready") {
              ...
              if (isEditing) {
                console.log(
                  "EditablePreviewPanel: Iframe ready, applying pending edit state."
                );
          ...
          case "commandResponse":
            console.log(
              `EditablePreviewPanel: Received command response for '${data.command}':`,
              data
            );
            if (!data.success) {
              message.error(
                `Preview command '${data.command}' failed: ${
          ...
          case "error":
            console.error(
              "EditablePreviewPanel: Error message from iframe:",
              data.message
            );
          ...
          default:
            console.log(
              "EditablePreviewPanel: Unknown message type received from iframe:",
              data.type
            );
        }
      }
  ...
  const reloadIframe = () => {
    ...
    if (iframeRef.current && iframeRef.current.contentWindow) {
      ...
      console.warn(
        "EditablePreviewPanel: iframe ref or contentWindow not available for reload."
      );
  ...
  const requestDOM = () => {
    ...
    message.loading({
      content: "Getting content from preview...",
      key: "getDOM",
    }); // Show antd loading message
  ...
  // --- Render Logic ---
  const currentFile =
    files && files.length > activeFileIndex ? files[activeFileIndex] : null;
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-hidden">
        ...
        >
          ...
          <div
            className={`flex flex-col relative ${isCollapsed ? "hidden" : ""}`}
          >
            {(files.length === 0 && (
          ...
          <div className="flex flex-col border-l border-gray-700 relative">
            ...
            <div className="p-2 bg-gray-800 border-b border-gray-700 flex items-center space-x-2">
              <div
                className={`flex-grow flex items-center px-2 py-1 bg-gray-900 rounded-lg border ${
                  (isUrlFocused && "border-blue-500") || "border-gray-700"
                }`}
              >
                {/* Loading indicator inside URL bar */}
                {isLoading && (
                  <Spin size="small" className="px-1 text-gray-400" />
                )}
                {!isLoading && <EyeOutlined className="px-1 text-gray-400" />}{" "}
                {/* Placeholder icon */}
                <input
                  type="url"
                  value={previewUrl}
                ...
                <Tooltip title="Reload Preview">
                  <button
                    ...
                    disabled={
                      isLoading || !iframeSrc || iframeSrc === "about:blank"
                    }
                  >
                    <ReloadOutlined />
                  </button>
              ...
              {iframeSrc && iframeSrc !== "about:blank" ? (
                <iframe
                  ...
                  onError={(e) => {
                    ...
                    message.error(
                      "Failed to load the preview content. Check the URL and browser console."
                    );
                    setIsLoading(false); // Stop loading on error
                    setIsIframeReady(false);
                  }}
                  onLoad={() => {
                    console.log(
                      "EditablePreviewPanel: Iframe onLoad event fired."
                    );
