...
interface AgenticFilterSuggestionMessageProps {
  message: MessageProps;
}

// 主组件，根据消息类型选择适当的子组件
const AgenticFilterSuggestionMessage: React.FC<
  AgenticFilterSuggestionMessageProps
> = ({ message }) => {
  // 检查消息是否为流式消息
  const isStreamingMessage =
    message.type === "STREAM" &&
    message.metadata?.stream_out_type === "command_suggestion";
...
// 流式命令消息组件，包含折叠/展开功能
const StreamingCommandMessage: React.FC<{
  message: MessageProps;
  isCompleted: boolean;
}> = ({ message, isCompleted }) => {
    ...
    <div className="message-font">
      <div className="message-title">
        ...
        >
          {isCollapsed ? (
            <svg
              className="message-toggle-icon text-yellow-400"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
        ...
        <span className="text-yellow-400 message-title-text">
          {message.isStreaming
            ...
            : getMessage("processingComplete")}
          ({getMessage("agenticFilterContext")})
        </span>
...
// 常规命令消息组件，正常显示内容
const RegularCommandMessage: React.FC<{ message: MessageProps }> = ({
  message,
}) => {
    ...
    <div className="message-font">
      <div className="message-title">
        ...
        <span className="text-blue-400 message-title-text">
          {getMessage("agenticFilterCommandChoose", {
...
export default AgenticFilterSuggestionMessage;
