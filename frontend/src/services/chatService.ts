...
export class ChatService extends EventEmitter {
  ...
  private startEventStream() {
    ...
    this.eventSource = new EventSource(`/api/chat-command/events?event_file_id=${this.eventFileId}`);
  ...
  private handleEvent(event: AutoCommandEvent) {
    ...
    if (event.event_type === 'STREAM') {
      if (this.lastEventType !== 'STREAM') {
        ...
        messageId = this.currentStreamMessageId || `chat-${uuid}-${timestamp}-${this.messageId++}`;
      }
    } else {
  ...
  private handleStreamEvent(event: AutoCommandEvent, messageId: string) {
    ...
    if (existingMessage) {
      ...
      if (content.state === 'complete') {
        ...
        console.log('ChatService: Emitting existing message:', existingMessage.type, existingMessage.id);
